...
(function() {
  var addFakePOSTags, addIdsToHierarchy, addSentence, addSentences, arrayToObj, callOnceElementAvailable, callbackParseHierarchy, callbackPartList, deserializeArray, getChildrenOfId, getMaxDepth, getParseHierarchyAndTranslationsForLang, getSentencePartList, getTranslationForLang, getUrlParameters, hierarchyToTerminals, hierarchyWithIdToTerminals, initializeHover, initializePopup, insertScript, makeDivs, objToArray, openTranslationPopup, renderSentence, root, serializeArray, submitTranslation, synthesizeSpeech, terminalElements, updateTranslation;
  ...
  terminalElements = function(contentHierarchy) {
    var child, output, terminalElement, _i, _j, _len, _len1, _ref;
    output = [];
    if (typeof contentHierarchy[0] === typeof '') return [contentHierarchy];
    for (_i = 0, _len = contentHierarchy.length; _i < _len; _i++) {
      child = contentHierarchy[_i];
      _ref = terminalElements(child);
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        terminalElement = _ref[_j];
  ...
  addSentence = root.addSentence = function(sentence, lang, renderTarget, options, callback) {
    if (options == null) options = {};
    return addSentences([sentence], lang, renderTarget, options, callback);
  };
  ...
  getTranslationForLang = root.getTranslationForLang = function(sentence, lang, callback) {
    return getParseHierarchyAndTranslationsForLang(sentence, lang, function(errors, currentPair) {
      return callback(currentPair[1][sentence]);
    });
  };

  getSentencePartList = root.getSentencePartList = function(sentence, lang, callback) {
    var basePath, callNum;
    if (root.isMTurk == null) {
      ...
      callbackPartList.push(function(resultData) {
        return callback(resultData);
      });
      basePath = '/getPartList';
      if (root.serverLocation.indexOf('heroku') !== -1) {
      ...
      return insertScript(root.serverLocation + basePath + '?sentence=' + encodeURI(sentence) + '&lang=' + encodeURI(lang) + '&callback=callbackPartList[' + callNum + ']');
    }
  };

  getParseHierarchyAndTranslationsForLang = function(sentence, lang, callback) {
    var basePath, callNum;
    if (root.isMTurk == null) {
      ...
      callbackParseHierarchy.push(function(resultData) {
        var currentPair;
      ...
      return insertScript(root.serverLocation + basePath + '?sentence=' + encodeURI(sentence) + '&lang=' + encodeURI(lang) + '&callback=callbackParseHierarchy[' + callNum + ']');
    }
  };
  ...
  addSentences = root.addSentences = function(sentences, lang, renderTarget, options, doneCallback) {
    ...
    getParseHierarchyAndTranslationsForLangCurried = function(sentence, callback) {
      return getParseHierarchyAndTranslationsForLang(sentence, lang, callback);
    };
    return async.mapSeries(sentences, getParseHierarchyAndTranslationsForLangCurried, function(err, results) {
